...
import { getCurrentUserPlan } from '@/services/stripe'

export default async function Page() {
  const session = await auth()
  const plan = await getCurrentUserPlan(session?.user.id as string)
    ...
    <form action={createCheckoutSessionAction}>
      <Card>
        ...
        <CardFooter className="flex items-center justify-between border-t pt-6">
          ...
          <Button type="submit">Assine por R$ 9,00/mês</Button>
        </CardFooter>
      </Card>
    </form>
  )
